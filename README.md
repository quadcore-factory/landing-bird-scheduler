...
The public booking surfaces inherit the active theme's font and WordPress global style tokens (`--wp--preset--color--base`, `--wp--preset--color--contrast`, `--wp--preset--color--accent-5`, heading sizes, and spacing). A site can set `--lb-scheduler-accent` on the page or wrapper when its primary action color needs an explicit brand override; no page builder is required.
